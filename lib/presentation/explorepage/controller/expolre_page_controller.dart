...
class ExplorePageController with ChangeNotifier {
  ...
  fetchData(context, {String? searchList,}) async {
    isLoading = true;
    notifyListeners();
    ExpolrePageService.fetchDetails(searchList).then((value) {
      if (value['status'] == 1) {
        log(
            "explorepage controller=> Allexploreservice-> data${value["data"]}");
        exploreModal = ExploreModal.fromJson(value);
        Navigator.push(context, MaterialPageRoute(builder: (context)=>SearchDetailsScreen()));
        isLoading = false;
      } else {
